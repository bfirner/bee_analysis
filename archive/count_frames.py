...
parallel_command_count = ["/research/projects/grail/rmartin/bin/parallel", "--jobs", "60", "--results", "pcounts.csv", "ffprobe",
                          "-v", "error", "-select_streams", "v:0", "-count_frames", "-show_entries", "stream=nb_read_frames", "-of", "csv=p=0", ":::"]

parallel_command_noBg = ["/research/projects/grail/rmartin/bin/parallel", "--jobs", "15", "--results", "noBg.csv", "/koko/system/anaconda/envs/python38/bin/python3",
                         "/research/projects/grail/rmartin/analysis-results/code/bee_analysis6/RemoveBackground.py", "--input", "{}", "--output", "{}.avi", ":::"]
...
parallel_background = ["/research/projects/grail/rmartin/bin/parallel", "--jobs", "60", "--results", "bgresults.csv",
                       "/research/projects/grail/rmartin/bin/RemoveBackground.py", "--input", "{}", "--output", "{}.avi", ":::"]
...
def main():
    ...
    parser.add_argument('--dir', type=str,
                        help='Directory of videos.', default='.')
    parser.add_argument('--which', type=str,
                        help='which operation: <count|noBg>', default='count')
    parser.add_argument(
        '--bgalg', type=str, help='Background subtraction algorithm, KNN or MOG2.', default='MOG2')
    ...
    if (which == 'count'):
        ...
        cleanup_result = subprocess.run(
            cleanup_shell_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
