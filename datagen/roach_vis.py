...
def generate_video(data):
    df = pd.read_csv(data, sep="(\s{3,})",
                     skiprows=SKIP_N_ROWS, engine='python')
    df.drop(index=df.index[:REMOVE_N_ROWS], inplace=True)
    vid_filename = data.replace(".txt", ".avi")
    out_vid = cv2.VideoWriter(vid_filename, cv2.VideoWriter_fourcc(
        *'DIVX'), VID_FPS, (VID_WIDTH, VID_HEIGHT))
